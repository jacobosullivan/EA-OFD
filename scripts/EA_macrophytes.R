...
if (0) {
  ...
  metr_phy <- as_tibble(read.csv("Macrophytes/MACP_OPEN_DATA_METRICS.csv"))
  site_phy <- as_tibble(read.csv("Macrophytes/MACP_OPEN_DATA_SITE.csv"))
  taxa_phy <- as_tibble(read.csv("Macrophytes/MACP_OPEN_DATA_TAXA.csv"))
  ...
  dat_phy <- taxa_phy %>% # select relevant response data
    select(ANALYSIS_ID, TAXON_LIST_ITEM_KEY, PERCENTAGE_COVER_BAND)
  dat_phy <- dat_phy %>% # add corresponding sample ID
    left_join(subset(metr_phy, select=c(ANALYSIS_ID, SAMPLE_ID, SAMPLE_DATE), by="ANALYSIS_ID"))
  dat_phy <- dat_phy %>% # add corresponding site ID
    left_join(subset(metr_phy, select=c(SAMPLE_ID, SITE_ID), by="SAMPLE_ID"))
  dat_phy <- dat_phy %>% # add corresponding catchment
    left_join(subset(site_phy, select=c(SITE_ID, CATCHMENT, WATER_BODY, FULL_EASTING, FULL_NORTHING, WFD_WATERBODY_ID), by="SITE_ID"))
  dat_phy <- dat_phy %>% # add rank of taxonomic record
  ...
  dat_phy <- dat_phy[dat_phy$WFD_WATERBODY_ID %in% wfd_id$Waterbody.ID, ]
  
  dat_phy <- dat_phy %>%
    filter(TAXON_RANK %in% c("Species", "Genus"))
  dat_phy$site <- as.numeric(factor(dat_phy$SITE_ID))
  dat_phy$taxon <- as.numeric(factor(dat_phy$TAXON_LIST_ITEM_KEY))
  dat_phy$PERCENTAGE_COVER_BAND[is.na(dat_phy$PERCENTAGE_COVER_BAND)] <- 0
  dat_phy$year <- stringr::str_extract(dat_phy$SAMPLE_DATE, "\\d{4}")
  colnames(dat_phy)[grep("WFD_WATERBODY_ID", colnames(dat_phy))] <- "Waterbody.ID"
  dat_phy <- dat_phy %>%
    left_join(subset(wfd_id, select=c(River.basin.district, 
                                      ...
                                      Northing)),
              by = "Waterbody.ID")
  write.csv(dat_phy, "Macrophytes/MACP_DATA_CLEANED.csv", row.names=F)
  dat_phy$count <- dat_phy[,which(colnames(dat_phy) %in% c("TOTAL_ABUNDANCE", "PERCENTAGE_COVER_BAND", "UNITS_FOUND_OR_SEQUENCE_READS"))]
} else {
  dat <- read.csv("Macrophytes/MACP_DATA_CLEANED.csv")
}
...
if (0) {
  ...
  write.csv(dat_turnoverSOD_raw, "summary_data/macrophyte_dat_turnoverSOD.csv", row.names=F)
} else {
...
if (0) {
  ...
  dat_turnoverSOD <- read.csv("summary_data/macrophyte_dat_turnoverSOD.csv")
}
...
if (0) {
  ...
  write.csv(dat_fitTimescales_ranges, "summary_data/macrophyte_dat_fitTimescales_ranges.csv", row.names=F)
} else {
...
if (0) {
  ...
  write.csv(dat_richnessLocal, "summary_data/macrophyte_dat_richnessLocal.csv", row.names=F)
